...
namespace PlagueEngine.Rendering
{
    ...
    class DynamicInstancedMeshes
    {
        ...
        /// Constructor
        /****************************************************************************/
        public DynamicInstancedMeshes(ContentManager content, Renderer renderer)
        {
        ...
        public void Draw(Effect effect)
        {
            foreach (PlagueEngineModel model in data.Keys)
            {
                ...
                foreach (TexturesPack texturesPack in data[model].Keys)
                {
                    ...
                    if (instancesData.InstancesVertexBuffer != null) 
                    {
                        instancesData.InstancesVertexBuffer.Dispose();
                    }

                    instancesData.InstancesVertexBuffer = new DynamicVertexBuffer(renderer.Device,
                                                                                  InstanceVertexDeclaration,
                                                                                  instancesData.MeshComponents.Count,
                                                                                  BufferUsage.WriteOnly);

                    Matrix[] transforms = new Matrix[instancesData.MeshComponents.Count];
                    int i = 0;
                    foreach (MeshComponent mesh in instancesData.MeshComponents)
                    {
                        transforms[i++] = mesh.GameObject.World;
                    }

                    instancesData.InstancesVertexBuffer.SetData(transforms, 0, instancesData.MeshComponents.Count, SetDataOptions.Discard);

                    renderer.Device.SetVertexBuffers(vertexBufferBinding,
                                                     new VertexBufferBinding(instancesData.InstancesVertexBuffer, 0, 1));
        ...
        private class InstancesData
        {
            ...
            public DynamicVertexBuffer  InstancesVertexBuffer = null;
            /************************************************************************/
